...
CREATE DATABASE library WITH TEMPLATE = template0 ENCODING = 'UTF8' LOCALE = 'Russian_Russia.1251';
...
CREATE TABLE lab3.replacement_acts (
    date date NOT NULL,
    replacement_book integer NOT NULL,
...
CREATE TABLE public.publishers (
    publisher_id integer NOT NULL,
    name character varying(20) NOT NULL,
    place_of_publication character varying(20) NOT NULL,
    CONSTRAINT check_name CHECK (((name)::text ~ '^[a-zA-Zа-яёА-ЯЁ -]+$'::text)),
    CONSTRAINT check_place_of_publication CHECK (((place_of_publication)::text ~ '^[a-zA-Zа-яА-Я -]+$'::text))
);
...
COPY lab3.authorships (authorship_id, author, book) FROM stdin;
1	1	1
3	7	3
4	8	4
5	9	5
...
COPY lab3.receipt_acts (receipt_act_id, source, date, employee_id, act_name) FROM stdin;
9	Покупка	2022-10-20	6	Товарная накл.№126
4	Обязательный экз.	2022-11-01	4	Акт о приеме док №2
7	Обязательный экз.	2022-10-05	5	Акт о приеме док №3
5	Покупка	2022-11-02	6	Товарная накл.№124
6	Покупка	2022-11-02	6	Товарная накл.№125
8	Обязательный экз.	2022-10-07	5	Акт о приеме док №5
\.
...
COPY lab3.replacement_acts (date, replacement_book, lost_book, employee, reader_card, act_id) FROM stdin;
2023-07-13	162	84	6	21	2
2023-07-13	163	95	5	27	3
2023-07-13	164	104	5	28	4
2023-09-07	166	111	5	19	5
2023-09-07	167	101	5	20	6
2023-09-07	168	81	6	18	7
2023-09-07	169	118	6	26	8
\.
...
COPY public.publishers (publisher_id, name, place_of_publication) FROM stdin;
\.
...
SELECT pg_catalog.setval('lab3.authorship_authorship_id_seq', 18, true);
...
--
-- Name: replacement_acts replacement_acts_publication_copies_inventory_number_fk; Type: FK CONSTRAINT; Schema: lab3; Owner: postgres
--

ALTER TABLE ONLY lab3.replacement_acts
    ADD CONSTRAINT replacement_acts_publication_copies_inventory_number_fk FOREIGN KEY (replacement_book) REFERENCES lab3.publication_copies(inventory_number);
